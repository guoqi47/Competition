...
def proprocess(file_path):
    ...
    if file_path == path_train:
        ...
        for i in range(len(lRes)):
            if len(lRes[i]) >= 3:
                clusterCenter = KMeans(n_clusters=3, random_state=0).fit(lRes[i]).cluster_centers_
                sortList=[]#用于簇的排序
                for i in range(3):
                    sortList.append(clusterCenter[i][-3])
                c = []
                index1 = sortList.index(max(sortList))#最大数的索引

                index3 = sortList.index(min(sortList))
                index2 = 3-index1-index3
                c.append(clusterCenter[index1])
                c.append(clusterCenter[index2])
                c.append(clusterCenter[index3])
                clusterCenters.append(c)
                yRes.append(y[i])
    else:
        clusterCenters = []
        for i in range(len(lRes)):
